...
fn fingerprint(key: &Key, content: &Value) -> Result<String, SnapshotError> {
    let bytes = serde_json::to_vec(&json!(["diplodocus/snapshot-record-v1", key, content]))?;
    Ok(fingerprint_bytes(&bytes).value)
}

fn records(snapshot: &Snapshot) -> Result<Vec<Record>, SnapshotError> {
...
fn content_fingerprint(snapshot: &Snapshot, records: &[Record]) -> Result<String, SnapshotError> {
...
pub(super) fn publish(snapshot: &Snapshot, path: &Path) -> Result<(), SnapshotError> {
    ...
    transaction.execute_batch(
        "CREATE TABLE manifest (
            ...
            storage_version INTEGER NOT NULL, ir_version INTEGER NOT NULL,
            encoding_version INTEGER NOT NULL, producer TEXT NOT NULL,
            content_fingerprint TEXT NOT NULL
        );
        CREATE TABLE records (
            kind TEXT NOT NULL, owner TEXT NOT NULL, id TEXT NOT NULL,
