...
| Output reduction | `src/execution/jupyter/output.rs` and child modules/tests | Incremental typed slots, page-wide updates and clearing, validator callbacks, plain-text MIME, normalized errors, diagnostics, and final figure validation are implemented. Rich validators and supervised invocation remain subsequent work. |
...
| Fragment parsing | `documents::parse_markdown_fragment`; `tests/markdown_fragments.rs` | Reuse inert parsing and attribution. URL, image, and cache validation are additional steps. |
