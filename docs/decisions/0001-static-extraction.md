...
contract](../spikes/static-extractor-contract.md).
  ...
  | Python source and stubs           | `ruff_python_parser`, `ruff_python_ast`, and `ruff_text_size` 0.0.13    | Select Python/stub mode and the package's target grammar, then build the configured module graph, exports, re-exports, declarations, and callable families. |
