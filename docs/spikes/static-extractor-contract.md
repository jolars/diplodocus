...
| `ruff_python_parser` | `0.0.13` | Parse `.py` and `.pyi`; record `source_type` for each input and the `target_version` selected from the package's declared Python requirement. |
| `ruff_python_ast` | `0.0.13` | Supply the typed syntax consumed by the semantic adapter. |
| `ruff_text_size` | `0.0.13` | Define Ruff's zero-based, half-open UTF-8 byte ranges. |
